...
// ─── Intelligence Tab ──────────────────────────────────────────────────────────
interface WeatherData { temperature_2m?: number; relative_humidity_2m?: number; wind_speed_10m?: number; uv_index?: number }
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="panel overflow-hidden">
          <div className="panel-header">Live Weather Context &nbsp;<span className="text-jade/60 normal-case tracking-normal">Open-Meteo</span></div>
          <div className="p-5">
            {weather ? (
              <div className="space-y-3">
                {[
                  { l: 'City temperature', v: `${weather.temperature_2m?.toFixed(1)}°C`, note: `node canopy: ${node.temperature_c}°C` },
                  { l: 'City humidity',    v: `${weather.relative_humidity_2m}%`,         note: `root zone: ${node.humidity_pct}%` },
                  { l: 'Wind speed',       v: `${weather.wind_speed_10m} km/h` },
                  { l: 'UV (city)',        v: String(weather.uv_index?.toFixed(1)),        note: `node base: ${node.uv_index}` },
                ].map(row => (
            ...
            ) : <p className="font-mono text-[10px] text-muted animate-pulse">Fetching...</p>}
          </div>
        </div>

        <div className="panel overflow-hidden">
          <div className="panel-header">Satellite Intelligence &nbsp;<span className="text-jade/60 normal-case tracking-normal">NASA FIRMS</span></div>
          <div className="p-5 space-y-3">
            {[
              { l: 'Fire hotspots (Mexico, 24h)', v: String(fires), c: fires > 20 ? '#C0392B' : fires > 5 ? '#B85C00' : '#2E7D6B' },
              { l: 'Pathogen risk (node)',         v: `${(node.pathogen_risk*100).toFixed(0)}%`, c: node.pathogen_risk > 0.6 ? '#C0392B' : '#B85C00' },
              { l: 'Biodiversity vs avg',          v: node.biodiversity_score < 0.5 ? `${(node.biodiversity_score*100).toFixed(0)}% ↓ below avg` : 'Within range', c: node.biodiversity_score < 0.5 ? '#C0392B' : '#2E7D6B' },
              { l: 'Composite risk',               v: `${((1-node.health_score)*100).toFixed(0)}%`, c: healthColor(node.health_score) },
            ].map(row => (
        ...
        <div className="p-5">
          {analysis ? (
            ...
            <div className="text-center py-8">
              <p className="font-mono text-[10px] text-muted mb-4">No analysis yet. Run the agent to see intelligence output.</p>
              <button onClick={onRun} disabled={loading}
...
export default function Dashboard() {
  ...
  const [log, setLog] = useState<LogEntry[]>([
    { ts: '06:47:00', type: 'INIT',     msg: 'ARVI network online — 6 EVVM contracts on Anvil',    color: '#2E7D6B' },
    { ts: '07:04:00', type: 'IDENTITY', msg: 'ERC-8004 registered — Base Mainnet · 0xb8623d...',    color: '#888' },
    { ts: '07:24:00', type: 'ANALYSIS', msg: 'Chapultepec plague confirmed — 94% confidence',       color: '#C0392B' },
  ...
  const runAgent = useCallback(async () => {
    ...
    setStage('analyze')
    addLog('ANALYZE', 'LLM analysis — Bankr Gateway', '#888')
    try {
    ...
    <div className="min-h-screen bg-canvas text-ink flex flex-col">
      ...
      <header className="shrink-0 border-b border-line bg-white z-10">
        <div className="px-6 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-5">
            ...
            <span className="font-mono text-[10px] text-muted/50 hidden md:block">{now} UTC</span>
            <Link href="/atlas" className="font-mono text-[11px] text-muted hover:text-jade transition-colors hidden md:block">Atlas ○</Link>
          </div>
        </div>
      </header>
      ...
      <div className="flex-1 flex overflow-hidden">
            ...
            {ARVI_NODES.map(node => {
              ...
              return (
                <button key={node.node_id} onClick={() => setActiveNode(node)}
                  className={`w-full text-left px-4 py-4 border-b border-line transition-all ${isActive ? 'bg-jade-light border-l-2' : 'hover:bg-canvas'}`}
        ...
        <main className="flex-1 flex flex-col overflow-hidden">
          {/* Node header + tabs */}
          <div className="border-b border-line bg-white px-6 pt-5 pb-0">
            <div className="flex items-start justify-between mb-4">
              <div>
                <p className="font-mono text-[10px] text-muted mb-1">{activeNode.ens} · {activeNode.zone}</p>
                <h2 className="font-serif text-2xl text-ink">{activeNode.location}</h2>
                <p className="font-mono text-[10px] text-muted mt-1">{activeNode.dominant_species} · {activeNode.trees_monitored} trees</p>
              </div>
              <button onClick={runAgent} disabled={loading}
                className="btn-jade disabled:opacity-40 flex items-center gap-2 mt-1">
                {loading
